...
jQuery(function($) {
	...
	const openModalBtn = $('#open-privacy-modal');
	const closeModalBtn = $('#close-modal');
	const returnToBannerBtn = $('#return-to-banner');
	const acceptCookiesBtn = $('#accept-cookies');
	const declineCookiesBtn = $('#decline-cookies');
	...
	// Retornar ao banner de cookies
	returnToBannerBtn.on('click', function() {
		privacyModal.fadeOut();
	});
	...
	document.addEventListener("DOMContentLoaded", function () {
		...
		const consent = localStorage.getItem("cookieConsent");
		if (!consent) {
			cookieBanner.classList.add("show"); // Exibe o banner
		}
	
		// Aceitar cookies
		acceptCookiesBtn.addEventListener("click", function () {
			localStorage.setItem("cookieConsent", "accepted");
			cookieBanner.classList.remove("show"); // Esconde o banner
		});
	...
	document.addEventListener("DOMContentLoaded", function () {
		...
		const returnToBannerBtn = document.getElementById("return-to-banner");
	
		// Abrir modal
		openModalBtn.addEventListener("click", function (e) {
			e.preventDefault();
			modal.style.display = "flex"; // Flex para centralizar o modal
		});
		...
		// Retornar ao banner e fechar modal
		returnToBannerBtn.addEventListener("click", function () {
			modal.style.display = "none";
		});
	...
	document.addEventListener('DOMContentLoaded', () => {
        ...
        const mainMenu = document.querySelector('.bottom-header nav'); // Menu principal
        const dropdownToggles = document.querySelectorAll('.drop-toggle'); // Links de dropdown
        const dropdowns = document.querySelectorAll('.drop'); // Menus dropdown

        // Alternar menu hambúrguer em dispositivos menores
        menuToggle.addEventListener('click', () => {
          mainMenu.classList.toggle('active');
        });

        // Alternar dropdown em dispositivos móveis
        dropdownToggles.forEach((toggle) => {
          toggle.addEventListener('click', (event) => {
            if (window.innerWidth <= 992) { // Apenas em telas pequenas
              event.preventDefault(); // Previne comportamento padrão do link
              const dropdown = toggle.nextElementSibling; // Dropdown relacionado
        ...
        // Fecha o menu hambúrguer e dropdowns ao redimensionar para desktop
        window.addEventListener('resize', () => {
          if (window.innerWidth > 992) {
            mainMenu.classList.remove('active'); // Fecha o menu hambúrguer
            dropdowns.forEach((drop) => drop.classList.remove('open')); // Fecha todos os dropdowns
          }
        });

        // Comportamento de hover para dropdowns em desktops
        dropdownToggles.forEach((toggle) => {
        ...
        // *** Código para o comportamento sticky da bottom-header ***
